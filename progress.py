...
------------------------------------------------------------------------------
Usage: from progress import ProgressMeter
'''
    ...
    def _get_meter(self, *args, **kw):
        ...
        if self.count >= self.total:
            ...
            if len(self.estimated_duration) >= 1:
                ...
                minutes, seconds = divmod(remainder,60)
                if est_dur < 60:
                    est = '%02.f sec' % seconds
                elif est_dur >= 60 and est_dur < 3600:
                    est = '%02.f min %02.f sec' % (minutes, seconds)
                else:
                    est = '%.f hr %02.f min %02.f sec' % (hours, minutes, seconds)
                return ' [%s-%s] %d%%  %s left' % (bar, pad, perc, est)
            else:
                return ' [%s-%s] %d%%  Starting...' % (bar, pad, perc)
